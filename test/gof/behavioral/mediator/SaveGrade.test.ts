...
test("Deve salvar a nota do aluno e calcular a media", async function () {
  ...
  const outputGetAverage = await getAverage.execute(studentId);
  expect(outputGetAverage.average).toBe(9);
});

test("Deve salvar a nota do aluno e calcular a media usando mediator", async function () {
  const studentId = Math.round(Math.random() * 100000);
  const gradeRepository = new GradeRepositoryDatabase();
  const averageRepository = new AverageRepositoryDatabase();
  const calculateAverage = new CalculateAverage(
