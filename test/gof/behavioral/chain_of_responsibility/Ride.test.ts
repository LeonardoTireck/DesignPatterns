...
beforeEach(() => {
  ...
  const overnightFareCalulator = new OvernightFareCalculator(
    sundayFareCalculator,
  );
  const normalFareCalculator = new NormalFareCalculator(overnightFareCalulator);
  ride = new Ride(normalFareCalculator);
});

test("Deve calcular o valor da corrida no horario normal", function () {
  ride.addSegment(10, new Date("2021-03-01T10:00:00"));
  ride.calculateFare();
  expect(ride.getFare()).toBe(21);
});

test("Deve calcular o valor da corrida a noite", function () {
